    ...
    for i, file in enumerate(files):
        ...
        with file.open("r", encoding="utf-8") as csvfile:
            ...
            for row_num, row in enumerate(csv_reader, start=1):
                ...
                document = Document(page_content=str(row), metadata={"source": str(file), "row_num": row_num})
                
                # Apply post-processing and split
                document = _post_process_document(document)
                chunks = _split_document(document)

                if constants.PARSING_WRITE_CHUNKS_TO_FILE:
                    chunked_file = file.with_name(f"{file.stem}_row{row_num}{constants.PARSING_CHUNKED_FILE_SUFFIX}")
